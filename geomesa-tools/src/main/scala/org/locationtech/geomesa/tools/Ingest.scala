...
class Ingest() extends Logging with AccumuloProperties {
  ...
  def runIngestJob(config: IngestArguments, fileSystem: String, password: String): Unit = {
    ...
    // create data store schema outside of map-reduce if we are not in a dry run
    if (!config.dryRun)
...
object Ingest extends App with Logging with GetPassword {
  ...
  private def getFileSystemMethod(path: String): String = path.toLowerCase.startsWith("hdfs") match {
    case true => "hdfs"
    case _    => "local"
  }
